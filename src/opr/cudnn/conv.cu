...
shared_ptr<TensorStorage> conv2d_forward(shared_ptr<TensorStorage> data, shared_ptr<TensorStorage> weight) {
...
struct Conv2DOpBackwarFunc: BackwardFunc {
    static std::shared_ptr<BackwardFunc> make(shared_ptr<GraphNode> x, shared_ptr<GraphNode> y){
        shared_ptr<BackwardFunc> func = make_shared<Conv2DOpBackwarFunc>();
...
Tensor conv2d(Tensor& data, Tensor& weight) {
    Tensor res(conv2d_forward(data.m_storage, weight.m_storage));
    ...
    if(data.m_need_grad || weight.m_need_grad || data.m_require_grad || weight.m_require_grad) {
        ...
        shared_ptr<BackwardFunc> func = Conv2DOpBackwarFunc::make(x_node, y_node);
